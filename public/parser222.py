...
def parse_html_statut(filepath):
    ...
    # ZMIANA TUTAJ: Poprawiony sposób znajdowania punktu startowego
    start_anchor = soup.find('a', attrs={'name': 'bookmark0'})
    first_chapter_heading = start_anchor.find_parent('h3') if start_anchor else None

    if not first_chapter_heading:
        print("Krytyczny błąd: Nie znaleziono punktu startowego ('bookmark0'). Parsowanie treści statutu zostało przerwane.")
        return {}
...
if __name__ == "__main__":
    ...
    script_name = 'testparser.py' # Zmień, jeśli nazwa pliku jest inna

    try:
        # Zmieniamy nazwę pliku, żeby nie nadpisać samego skryptu
        if script_name == input_file or script_name == output_file:
             print(f"Błąd: Nazwa pliku wejściowego ('{input_file}') lub wyjściowego ('{output_file}') jest taka sama jak nazwa skryptu ('{script_name}'). Zmień nazwę pliku skryptu i spróbuj ponownie.")
        else:
