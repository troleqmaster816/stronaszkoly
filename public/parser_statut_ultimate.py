...
def parse_html_statut(filepath):
    ...
    # Sekcje 1, 2, 3 (Metadane, Podstawa Prawna, Spis Treści) - bez zmian
    ...
    # ---- 4. Parsowanie głównej treści (Z POPRAWKĄ) ----
    chapters = []
    ...
    # Iterujemy po wszystkich tagach, które mogą definiować strukturę
    for tag in first_chapter_heading.find_all_next(['h3', 'h4', 'p', 'ol']):
        ...
        # Akapit lub podnagłówek
        if tag.name == 'p' and tag.get_text(strip=True):
            # *** ZMIANA TUTAJ: KLUCZOWY WARUNEK ZAPOBIEGAJĄCY DUPLIKACJI ***
            # Jeśli ten akapit jest częścią elementu listy, pomijamy go,
            # ponieważ zostanie on przetworzony przez logikę obsługi list.
